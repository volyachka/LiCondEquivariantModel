...
def test_sevennet_equivariance_energies():
    ...
    for batch in tqdm(dataloader):
        ...
        #energy check
        np.testing.assert_allclose(SevennetPredictor.predict(initial_atoms_arr)['energy'], SevennetPredictor.predict(rotated_atoms_arr)['energy'], atol=1e-6, rtol=1e-6)
...
def test_sevennet_equivariance_forces():
    ...
    for batch in tqdm(dataloader):
        ...
        for force_initial, force_rotated, wigner_D in zip(SevennetPredictor.predict(initial_atoms_arr)['forces'], SevennetPredictor.predict(rotated_atoms_arr)['forces'], wigner_matrices_arr):
            np.testing.assert_allclose(force_initial @ wigner_D.T, force_rotated, atol=1e-3, rtol=1e-5)
